...
void CTestAdoDlg::OnAdd() 
{
	...
	set.AddNew();
	// 3 设置该记录的字段
	set.SetFieldValue(0,"6");
	set.SetFieldValue(1,"66");
	set.SetFieldValue(2,"456");
	set.SetFieldValue(3,"4567");
	set.SetFieldValue(4,"45678");
	// 4 更新到数据库
	set.Update();
	// 5 重新显示数据
	Show(&set);
...
void CTestAdoDlg::OnEdit() 
{
    CAdoRecordset set;
	set.OpenTable("TBL_MSG",&m_db);
	set.MoveLast();
	set.SetFieldValue(1,"123");
	set.SetFieldValue(3,"123");
	set.SetFieldValue(4,"123");
	set.Update();
	Show(&set);
...
{
	// 1 执行增加记录的sql语句
	m_db.OpenSql("insert into TBL_MSG(msg_id,msg_sender) values('7','567')");
	// 2 删除和修改的Sql语句...
...
void CTestAdoDlg::OnTrans() 
{
	...
	m_db.BeginTrans();
	// 2 执行删除操作
    m_db.OpenSql("delete from TBL_MSG");
	// 3 打开表查看清空的效果
	OnOpenTable();
	// 4 提示
	MessageBox("回滚事务！");
