...
{   //创建UDP服务器
    ...
    bind( m_SvrSocket, (struct sockaddr *)&addr,
        sizeof( addr ) );
    //创建服务器线程
...
UINT CUserServer::UserThread( LPVOID pParam )
{
    ...
    while( 1 )
    {
        ...
        recvfrom( pThis->m_SvrSocket, (char *)&packet, 
            sizeof( packet ), 0, 
            (struct sockaddr *)&addr, &nLen );
        ...
        case NETCMDID_USERQUIT:
            pThis->OnUserQuit( 
                (LPUSERQUIT)&packet, pszIP );
            break;
        case NETCMDID_USERCHAT:
            pThis->OnUserChat( 
                (LPUSERCHAT)&packet, pszIP );
            break;
        }
    }
    return 0;
...
BOOL CUserServer::OnUserBroadcast( LPUSERBROADCAST pUserBroadcast, 
                                   CHAR * pszIP )
{   //将用户添加到 UserView 窗口
    if( m_pUserView != NULL )
    {
...
BOOL CUserServer::OnUserQuit( LPUSERQUIT pUserQuit, 
                              CHAR * pszIP )
{
    if( m_pUserView != NULL )
    {
