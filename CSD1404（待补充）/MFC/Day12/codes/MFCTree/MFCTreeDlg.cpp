...
void CMFCTreeDlg::InitTreeCtrl()
{
   ...
   //3 插入树节点
   HTREEITEM hRoot=m_wndTree.InsertItem("达内集团",
	   0,1);
   HTREEITEM hTT=m_wndTree.InsertItem("天坛中心",
	   2,3,hRoot);
   HTREEITEM hTAM=m_wndTree.InsertItem("天安门中心",
	   2,3,hRoot);
   HTREEITEM hZTF=m_wndTree.InsertItem("总统府中心",
	   2,3,hRoot,hTT);
   HTREEITEM hCSD=m_wndTree.InsertItem("csd1404班",
	   2,3,hTT);
   //4 设置节点高度
   m_wndTree.SetItemHeight(25);
   //5 设置节点的展开状态
   m_wndTree.Expand(hRoot,TVE_EXPAND);
   m_wndTree.Expand(hTT,TVE_EXPAND);
